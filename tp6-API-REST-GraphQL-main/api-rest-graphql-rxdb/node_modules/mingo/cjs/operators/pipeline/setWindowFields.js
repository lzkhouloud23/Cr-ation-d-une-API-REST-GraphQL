...
const $setWindowFields = (collection, expr, options) => {
  ...
  for (const outputExpr of Object.values(expr.output)) {
    ...
    (0, import_util.assert)(
      !!context.getOperator(import_internal.OpType.WINDOW, op) || !!context.getOperator(import_internal.OpType.ACCUMULATOR, op),
      ...
      const config = {
        ...
        func: {
          left: options.context.getOperator(
            import_internal.OpType.ACCUMULATOR,
            op
          ),
          right: options.context.getOperator(
            import_internal.OpType.WINDOW,
            op
          )
        },
        args: outputExpr[op],
