...
const $dateFromString = (obj, expr, options) => {
  const args = (0, import_internal.computeValue)(obj, expr, null, options);
  args.format = args.format || import_internal2.DATE_FORMAT;
  args.onNull = args.onNull || null;
  let dateString = args.dateString;
  if ((0, import_util.isNil)(dateString)) return args.onNull;
  const separators = args.format.split(import_internal2.DATE_FORMAT_SEP_RE);
  separators.reverse();
  const matches = args.format.match(import_internal2.DATE_FORMAT_SYM_RE);
  const dateParts = {};
  let expectedPattern = "";
