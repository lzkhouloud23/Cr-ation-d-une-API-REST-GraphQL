...
const $dateToString = (obj, expr, options) => {
  ...
  for (let i = 0, len = matches.length; i < len; i++) {
    ...
    (0, import_util.assert)(
      formatSpecifier in import_internal2.DATE_SYM_TABLE,
      `$dateToString: invalid format specifier ${formatSpecifier}`
    );
    const { name, padding } = import_internal2.DATE_SYM_TABLE[formatSpecifier];
    const fn = DATE_FUNCTIONS[formatSpecifier];
    let value;
    if (fn) {
      value = (0, import_internal2.padDigits)(fn(obj, date, options), padding);
    } else {
      switch (name) {
        case "timezone":
