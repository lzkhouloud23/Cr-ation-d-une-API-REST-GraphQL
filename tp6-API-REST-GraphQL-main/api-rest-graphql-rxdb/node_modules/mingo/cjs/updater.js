...
function getModifiedFields(pipeline, oldDoc, newDoc) {
  ...
  for (const key of stageFieldsSet) {
    if (pathValidator.add(key) && !(0, import_util.isEqual)((0, import_util.resolve)(newDoc, key), (0, import_util.resolve)(oldDoc, key))) {
      modifiedFields.push(key);
    }
  }
  for (const key of Object.keys(oldDoc)) {
    if (stageFieldsSet.has(key)) continue;
    if (!pathValidator.add(key) || !(0, import_util.isEqual)(newDoc[key], oldDoc[key])) {
      modifiedFields.push(key);
    }
  }
